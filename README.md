...
1. \dt
1. SELECT relname, relrowsecurity, relforcerowsecurity FROM pg_class WHERE relname IN ('users', 'portfolios', 'portfolio_items', 'chat_mes
sages', 'analysis_history', 'chat_sessions');
1. SELECT * FROM pg_policies WHERE tablename IN ('users', 'portfolios', 'portfolio_items', 'chat_messages', 'analysis_history', 'chat_sessions');
1. \d portfolios
1. \d portfolio_items
